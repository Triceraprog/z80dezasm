from z80dezasm.z80tools import *
...
class DecodeEDPrefixTestCase(unittest.TestCase):
    ...
    def test_decode_of_out_16_bits(self):
        memory = [0xED, 0x41]
        expected = ("OUT", P_REGISTER_PAIR_INDIRECT, REG_BC, P_REGISTER, REG_B)
        self.assertEqual(expected, decode(memory))

        memory = [0xED, 0x79]
        expected = ("OUT", P_REGISTER_PAIR_INDIRECT, REG_BC, P_REGISTER, REG_A)
        self.assertEqual(expected, decode(memory))
    ...
    def test_decode_of_adc_sbc_16_bits(self):
        memory = [0xED, 0x4A]
    ...
    def test_decode_of_ld_16_bits_immediate_address(self):
        ...
        memory = [0xED, 0x5B, 0x8E, 0x48]
        expected = ("LD", P_REGISTER_PAIR, REG_DE, P_IMMEDIATE_16_INDIRECT, 0x488E)
        self.assertEqual(expected, decode(memory))
        size = decode_full(memory)[-1]
        self.assertEqual(4, size)
...
class DecodeDDPrefixTestCase(unittest.TestCase):
    ...
    def test_ld_with_dd_prefix(self):
        ...
        memory = [0xDD, 0x4e, 0x08]
        expected = ("LD", P_REGISTER, REG_C, P_REGISTER_INDEXED, (REG_IX, 8))
        self.assertEqual(expected, decode(memory))
        size = decode_full(memory)[-1]
        self.assertEqual(3, size)

        memory = [0xDD, 0x21, 0xFA, 0x47]
        expected = ("LD", P_REGISTER_PAIR, REG_IX, P_IMMEDIATE_16, 0x47FA)
        self.assertEqual(expected, decode(memory))
        size = decode_full(memory)[-1]
        self.assertEqual(4, size)
...
class DecodeFDPrefixTestCase(unittest.TestCase):
    def test_ignore_dd_if_another_conflicting_prefix(self):
        ...
        memory = [0xFD, 0xE5]
        expected = ("PUSH", None, None, P_REGISTER_PAIR, REG_IY)
        self.assertEqual(expected, decode(memory))
        size = decode_full(memory)[-1]
        self.assertEqual(2, size)
