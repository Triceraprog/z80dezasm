...
def load_rom_with_comments():
    ...
    starting_addresses = get_starting_addresses(comments.all_directives(), data_skip_ranges)

    rom = Rom(rom_raw_content)
    rom = analysis(rom, starting_addresses, data_skip_ranges)
...
def get_starting_addresses(directives, data_skip_ranges=[]):
    ...
    for address, size in data_skip_ranges:
        starting_addresses.append(address + size)

    return starting_addresses


def get_data_skip_ranges(directives):
    # Data Skip are data than are intertwined in code parts with PC manipulation
    # They generate a starting address after their range
    data_ranges = []
    for address, directives in directives:
        for directive in directives:
            if directive.startswith("DATASKIP"):
                closing_parenthesis = directive.find(")")
